    ...
    <form onSubmit={handleTokenVerification} className='bg-white rounded-lg shadow-md w-full max-w-sm mx-auto'>
            ...
            <div className='p-6'>
                <div className='mb-4'>
                    ...
                    <input label="TOTP" type="text" value={otp} onChange={(e) => setOtp(e.target.value)} className='w-full p-2 border rounded mt-2' placeholder='Enter your TOTP' required />
                </div>
                {error && <p className='text-red-500 text-sm mb-3'>{error}</p>}
                <button type='submit' className='w-full bg-blue-500 text-white py-2 rounded-md mb-3'>Verify TOTP</button>
                <button type='button' className='w-full bg-slate-500 text-white py-2 rounded-md' onClick={handleReset}>Reset 2FA</button>
